...
int main()
{
    ...
    cout << "--- Iterate array with objects:" << endl;
    ...
    // Problem: polymorphism doesn't work well.
    // getSound() and getString() of base class Animal always called, regardless of the object

    // Solution: array should contain pointers to the objects
    cout << "--- Iterate array with pointers to objects:" << endl;
    Animal *animals2[] = {&cat, &dog, &bird, &wildBird, &animal, &pet};
    for (int i = 0; i < 6; i++)
    {
    ...
    // print(animal3);
    delete animal3;
    cout << "--- Object in heap (end)" << endl << endl << endl;

    // Problem: still doesn't work well. No change visible...

    // Solution: make the overridable methods virtual and/or override
