...
static uint8_t getal = 138, *getalPointer;
...
int main(void)
{
	...
	while (1)
	{	
		sprintf(text, "Waarde van getal: %d.\r\n", getal);
		StringToUsart2(text);
		
		sprintf(text, "Adres van getal: %d.\r\n", (uint32_t)&getal);
		StringToUsart2(text);
		
		getalPointer = &getal;
		
		sprintf(text, "Waarde van getalPointer: %d.\r\n", (uint32_t)getalPointer);
		StringToUsart2(text);
		
		sprintf(text, "Adres van getalPointer: %d.\r\n", (uint32_t)&getalPointer);
		StringToUsart2(text);
		
		sprintf(text, "Waarde naar waar getalPointer wijst: %d.\r\n\r\n", *getalPointer);
		StringToUsart2(text);
		
		WaitForMs(5000);
		
		// Pas de waarde van getal aan via getalPointer...
		*getalPointer = 139;
	}
}
